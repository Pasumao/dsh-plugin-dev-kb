...
│   │                       cordis-api、cookbook、subsystems(42 个子系统页)…
...
7. **按需查阅** — `site/reference/`：
   ...
   - 子系统 `reference/subsystems/`（core / scope / session / tools / shell / filesystem / subagent / sandbox / approval … 共 42 页）
...
# 2. 运行站点投影器（需要 Node ≥ 22.19）
node --experimental-strip-types run-projector.ts   # 输出到 website/.generated/
# 3. 用 website/.generated/ 覆盖本插件的 kb/site/
# 4. 在本插件根目录重新生成索引与 INDEX.md
node scripts/rebuild-index.mjs
```

站点投影逻辑为仓库内 `website/docs.ts`（发布清单）+ `scripts/project-doc-site.ts`（链接重写/图片复制）。

## 统计

- 站点页面：168 个 Markdown（中文 84 + 英文 84），4 张图片
- 补充文档：50 个 Markdown（`extra/`）
- 来源 commit：`99f6f02fecdb7dff40c3fbc9470f5907c29f74ca`（master）
